...
def finetune_model(model, config, config_paths, config_train, num_classes, logger, save_dir):
    """Train the model."""
...
def finetune(config: ExperimentationConfig):
    """Train the model."""
    ...
    model.load_state_dict(torch.load(f'{save_dir}/best_acc.pth'),  strict=False) # load the best model
    ...
    model.train()
    finetune_model(model, config, config_paths, config_train, num_classes, logger, save_dir)
